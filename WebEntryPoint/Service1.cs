...
namespace WebEntryPoint
{
    ...
    { 
        ...
        private static readonly NLogWrapper.ILogger _logger = LogManager.CreateLogger(typeof(Service1));
        ...
        private void CheckHealth()
        {
            ...
            if (ConfigurationManager.AppSettings.Get(Helpers.Appsettings.HostKey()) == null) throw new Exception(Helpers.Appsettings.HostKey() + " is not present in app.config");
            ...
            if (ConfigurationManager.AppSettings.Get(Helpers.Appsettings.SocketServerUrlKey()) == null) throw new Exception(Helpers.Appsettings.SocketServerUrlKey() + " is not present in app.config");
        ...
        protected override void OnStart(string[] args)
        {
            ...
            string httpsLocalAddress = "https://local.entrypoint:9000/";
            string httpsExternalAddress = "https://ec2-52-57-195-49.eu-central-1.compute.amazonaws.com:9000/";
            string httpLocalAddress = "http://local.entrypoint:9001/";

            StartOptions options = new StartOptions();
            options.Urls.Add(httpLocalAddress); 
            options.Urls.Add(httpsLocalAddress);
            options.Urls.Add(httpsExternalAddress);
            // Start OWIN host 
            _httpServer = WebApp.Start<HttpHost>(options);
            _logger.Info("OWIN http Server started!");

            _logger.Info("Starting socket server.");
            _socketServer = new WebSockets.SocketServer();
            //_socketServer.WireFleckLogging();

            _socketServer.Start(Helpers.Appsettings.SocketServerUrl().Replace("local.frontend", "0.0.0.0"));
